...
export default function OrderCard({ order, onPrintFile, onCompleteFile }: OrderCardProps) {
	...
	const orderTotal = order.files.reduce((sum, f) => sum + f.price, 0);
							...
							<div className="flex items-center space-x-3 min-w-0 w-full sm:w-auto sm:mr-auto">
								...
								<div className="min-w-0">
									...
									<p className="text-xs text-gray-500 font-mono-code mt-1">
										{file.pageCount} pg{file.pageCount !== 1 ? "s" : ""} &middot; {file.copies}x &middot;{" "}
										{file.colorMode} &middot; {file.doubleSided ? "Double" : "Single"}
										{file.price > 0 && <> &middot; ₹{file.price.toFixed(2)}</>}
									</p>
								</div>
							</div>
